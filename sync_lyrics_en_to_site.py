"""
Synchronise les paroles EN depuis Lyrics/EN vers lyrics-data-en.js
Adapté de sync_lyrics_to_site.py pour l'anglais
"""
import re
...
# Configuration
LYRICS_BASE_DIR = Path("G:/Mon Drive/01 BibleChantee/Lyrics/EN")
GITHUB_REPO_DIR = Path("C:/ScriptBible/bible-chantee")
LYRICS_DATA_FILE = GITHUB_REPO_DIR / "lyrics-data-en.js"
...
def collect_all_lyrics():
    ...
    for book_code, book_id in BOOK_CODES.items():
        ...
        # Trouver tous les fichiers de paroles EN
        files = sorted(book_dir.glob(f"{book_code}_*_EN.txt"))
        ...
        for file_path in files:
            ...
            match = re.search(r'_(\d+)_EN\.txt$', file_path.name)
            if not match:
                continue
...
def generate_lyrics_js(lyrics_data):
    """Génère le contenu du fichier lyrics-data-en.js"""
    js_content = "// Lyrics EN - Sung Bible\n"
    js_content += f"// Auto-generated: {datetime.now().strftime('%Y-%m-%d')}\n"
    js_content += "window.chapterLyricsEN = {\n"
...
def main():
    print("=== SYNCHRONISATION DES PAROLES EN ===\n")

    # 1. Collecter toutes les paroles
    print("[1/3] Collecting lyrics from EN folders...")
    ...
    print(f"\n  Total: {total_books} books, {total_chapters} chapters\n")
    ...
    print("[2/3] Generating lyrics-data-en.js...")
    ...
    try:
        ...
        print(f"  {total_books} books × {total_chapters} chapters")
        return True
    except Exception as e:
        print(f"  ERROR: {e}")
        return False
