...
const CSV_PATH = path.join(ROOT, "errors_IT_completes.csv");
const OUT_JSON = path.join(ROOT, "corrections_IT.json");
...
function buildPrompt(batch) {
  const items = batch.map(it =>
    `- ${it.book}:${it.chap}\n  FR: ${it.fr}\n  IT_CURRENT: ${it.itCurrent}`
  ).join("\n");

  return `
Return STRICT JSON only: { "01:6": "Titolo", ... } using exactly the keys provided.

Translate French Bible chapter song titles into natural Italian.
- Natural, idiomatic Italian (not word-for-word).
- Biblical tone where appropriate (e.g., "O Signore", "Patto", "Ecco").
- Title Case for proper nouns and key words.
- No French accents (ê, ë, û, ô), no French words ("Éternel", "Majesté", etc.).
- Fix mixed fragments like "nostro demeure" -> "La Nostra Dimora".
- Keep proper nouns (Bethel, El-Bethel, Abraham, etc.).
- Use Italian accents where appropriate (à, è, é, ì, ò, ù).
...
async function main() {
  ...
  for (let i = 0; i < items.length; i += BATCH_SIZE) {
    ...
    for (const [k, v] of Object.entries(obj)) {
      correctionsFlat[k] = normalizeItalian(v);
    }
  }
