...
const CSV_PATH = path.join(ROOT, "errors_DE_completes.csv");
const OUT_JSON = path.join(ROOT, "corrections_DE.json");
...
function buildPrompt(batch) {
  const items = batch.map(it =>
    `- ${it.book}:${it.chap}\n  FR: ${it.fr}\n  DE_CURRENT: ${it.deCurrent}`
  ).join("\n");

  return `
Return STRICT JSON only: { "01:6": "Titel", ... } using exactly the keys provided.

Translate French Bible chapter song titles into natural German.
- Natural, idiomatic German (not word-for-word).
- Biblical tone where appropriate (e.g., "O Herr", "Bund", "Siehe").
- Title Case for nouns (German capitalization rules).
- No French accents (é, è, ê, etc.), no French words ("Éternel", "Majesté", etc.).
- Fix mixed fragments like "Führe nos cœurs" -> "Führe unsere Herzen".
- Keep proper nouns (Bethel, El-Bethel, Abraham, etc.).
- Use German characters where appropriate (ä, ö, ü, ß).
...
async function main() {
  ...
  for (let i = 0; i < items.length; i += BATCH_SIZE) {
    ...
    for (const [k, v] of Object.entries(obj)) {
      correctionsFlat[k] = normalizeGerman(v);
    }
  }
