...
const CSV_PATH = path.join(ROOT, "errors_ES_completes.csv");
const OUT_JSON = path.join(ROOT, "corrections_ES.json");
...
function buildPrompt(batch) {
  const items = batch.map(it =>
    `- ${it.book}:${it.chap}\n  FR: ${it.fr}\n  ES_CURRENT: ${it.esCurrent}`
  ).join("\n");

  return `
Return STRICT JSON only: { "01:6": "Título", ... } using exactly the keys provided.

Translate French Bible chapter song titles into natural Spanish.
- Natural, idiomatic Spanish (not word-for-word).
- Biblical tone where appropriate (e.g., "Oh Señor", "Pacto", "He aquí").
- Title Case for proper nouns and key words.
- No French accents (é, è, ê, etc.), no French words ("Éternel", "Majesté", etc.).
- Fix mixed fragments like "devant el Eterno" -> "Delante del Eterno".
- Keep proper nouns (Bethel, El-Bethel, Abraham, etc.).
- Use Spanish accents where appropriate (á, é, í, ó, ú, ñ).
...
async function main() {
  ...
  for (let i = 0; i < items.length; i += BATCH_SIZE) {
    ...
    for (const [k, v] of Object.entries(obj)) {
      correctionsFlat[k] = normalizeSpanish(v);
    }
  }
