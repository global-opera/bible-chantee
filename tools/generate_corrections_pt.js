...
const CSV_PATH = path.join(ROOT, "errors_PT_completes.csv");
const OUT_JSON = path.join(ROOT, "corrections_PT.json");
...
function normalizePortuguese(s) {
  return String(s || "")
    .replace(/[']/g, "'")
    .replace(/[""]/g, '"')
    .replace(/\s+/g, " ")
...
function buildPrompt(batch) {
  const items = batch.map(it =>
    `- ${it.book}:${it.chap}\n  FR: ${it.fr}\n  PT_CURRENT: ${it.ptCurrent}`
  ).join("\n");

  return `
Return STRICT JSON only: { "01:6": "Título", ... } using exactly the keys provided.

Translate French Bible chapter song titles into natural Brazilian Portuguese (PT-BR).
- Natural, idiomatic Portuguese (not word-for-word).
- Biblical tone where appropriate (e.g., "Ó Senhor", "Pacto", "Eis").
- Title Case for proper nouns and key words.
- No French accents (è, ë, û, ô, î, ï), no French words ("Éternel", "Majesté", etc.).
- Fix mixed fragments like "Em la Vallée" -> "No Vale".
- Keep proper nouns (Bethel, El-Bethel, Abraão, etc.).
- Use Portuguese accents where appropriate (á, â, ã, à, é, ê, í, ó, ô, õ, ú, ü, ç).
...
async function main() {
  ...
  for (let i = 0; i < items.length; i += BATCH_SIZE) {
    ...
    for (const [k, v] of Object.entries(obj)) {
      correctionsFlat[k] = normalizePortuguese(v);
    }
  }
