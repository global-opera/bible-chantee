...
const LYRICS_PATH = path.join(ROOT, "lyrics-archive", "PT.json");
const TITLES_PATH = path.join(ROOT, "titles", "PT.json");
  ...
  for (const book of Object.keys(lyrics)) {
    ...
    for (const chap of Object.keys(lyrics[book])) {
      ...
      if (typeof content === "string" && content.includes("[TITRE]")) {
        // Get actual title from titles/PT.json
        ...
        if (actualTitle) {
          ...
          console.warn(`⚠ ${book}:${chap} - No title found in titles/PT.json`);
          notFound++;
        }
      }
    }
