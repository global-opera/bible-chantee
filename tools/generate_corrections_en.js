...
const CSV_PATH = path.join(ROOT, "errors_EN_completes.csv");
const OUT_JSON = path.join(ROOT, "corrections_EN.json");
...
function buildPrompt(batch) {
  const items = batch.map(it =>
    `- ${it.book}:${it.chap}\n  FR: ${it.fr}\n  EN_CURRENT: ${it.enCurrent}`
  ).join("\n");

  return `
Return STRICT JSON only: { "01:6": "Title", ... } using exactly the keys provided.

Translate French Bible chapter song titles into natural English.
- Natural, idiomatic English (not word-for-word).
- Biblical tone where appropriate (e.g., "O Lord", "Covenant", "Behold").
- Title Case.
- No French accents, no French words/fragments, no "à", "l'", "Éternel", etc.
- Fix mixed fragments like "our lieu of rencontre" -> "Our Meeting Place".
...
async function main() {
  ...
  for (let i = 0; i < items.length; i += BATCH_SIZE) {
    ...
    for (const [k, v] of Object.entries(obj)) {
      correctionsFlat[k] = normalizeEnglish(v);
    }
  }
