...
const CSV_OUT = path.join(ROOT, "errors_DE_completes.csv");

const ACCENTED_CHARS = "éèêëàâùûôîïçÉÈÊËÀÂÙÛÔÎÏÇ";
const reAccents = new RegExp("[" + ACCENTED_CHARS.replace(/[-/\\^$*+?.()|[\]{}]/g, "\\$&") + "]");

// French words that should not appear in German text
const FRENCH_WORDS = [
...
function main() {
  ...
  for (const book of Object.keys(de)) {
    for (const chap of Object.keys(de[book] || {})) {
      ...
      if (reAccents.test(title)) {
        ...
        errors.push({ book, chap, titleDE: title, titleFR: frSource });
        continue;
      }
  ...
  if (errors.length) {
    console.log(`AUDIT DE: ${errors.length} erreur(s) détectée(s)`);
    console.log("\nExemples (10 premiers):");
    errors.slice(0, 10).forEach((e, i) => {
      console.log(`${String(i+1).padStart(2,"0")}. ${e.book}:${e.chap} -> ${e.titleDE}`);
    });

    // Generate CSV
    const csvLines = ["Livre,Chapitre,Titre_DE_Actuel,Titre_FR_Source"];
    for (const e of errors) {
      const escapeCsv = (s) => {
        s = String(s || "").replace(/"/g, '""');
        return s.includes(",") || s.includes('"') || s.includes("\n") ? `"${s}"` : s;
      };
      csvLines.push(`${e.book},${e.chap},${escapeCsv(e.titleDE)},${escapeCsv(e.titleFR)}`);
    }
  ...
  console.log("AUDIT DE: OK (0 erreur)");
  process.exit(0);
}
